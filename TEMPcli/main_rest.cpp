...
const int LEVEL_WIDTH = 2240;
const int LEVEL_HEIGHT = 1280;

//Tile constants
const int TILE_WIDTH = 40;
const int TILE_HEIGHT = 40;
const int TOTAL_TILES = 1790;
	...
	if( e.type == SDL_KEYDOWN && e.key.repeat == 0 )
    {
        ...
        if (isYuluOn == false)
        {
            switch( e.key.keysym.sym )
            {
				...
				case SDLK_1: if (i==5) {printf("Yummy Noodles!!\n");}break;
				case SDLK_2: if (i==5) {printf("Tasty Burger!!\n");}break;
            }
        }
        else
        {
...
void Dot::information()
{
        ...
        if (i==407 || i==433)
        {
            printf("Press 1 to eat noodles , press 2 to eat burger!\n");
		}
}
...
bool setTiles( Tile* tiles[] )
{
    ...
    //Open the map
    std::ifstream map( "lazy_rest.map" );
...
bool touchesWall( SDL_Rect box, Tile* tiles[] )
{
    ...
    for( int i = 0; i < TOTAL_TILES; ++i )
    {
        ...
        bool collide = ( tiles[ i ]->getType() == TILE_BOTTOM ) || ( tiles[ i ]->getType() == TILE_BOTTOMRIGHT ) ;
        collide = ! collide;
