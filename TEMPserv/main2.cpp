...
std::string buildings[21][2] = {{"Jwalamukhi Hostel!","1"},
                                {"Aravali Hostel!","1"},
                                {"Karakoram Hostel!","1"},
                                {"Nilgiri Hostel!","1"},
                                {"Kumaon Hostel!","1"},
	...
	if( e.type == SDL_KEYDOWN && e.key.repeat == 0 )
    {
        ...
        else
        {
            switch( e.key.keysym.sym )
            {
                ...
                case SDLK_e: if (j==2){changeMoney() ;isYuluOn =false; }break;
            }
        }
...
{
    //Show the dot
	gDot2Texture.render( mBox.x - camera.x, mBox.y - camera.y );
}
	...
	{	
            ...
            {	
                ...
                while( !quit )
                {
                    ...
                    dot2.render( camera );
                    if (inmap){
                        gDot1Texture.render( dot2.mBox.x - camera.x, dot2.mBox.y - camera.y );}
